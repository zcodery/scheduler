...
function App() {
  ...
  const getViewDuration = () => {
    switch(viewMode) {
        case 'month': return ONE_DAY_MS * 30;
        case 'quarter': return ONE_DAY_MS * 90;
        case 'year': return ONE_DAY_MS * 365;
        default: return ONE_DAY_MS * 30;
    }
  };
      ...
      if (interaction.type === 'move' && interaction.initialRowOffset !== undefined && interaction.offsetMs !== undefined) {
          ...
          if (currentStaffIdx !== -1) {
             ...
             if (targetStaffId && targetStaffId !== newStaffData[currentStaffIdx].id) {
                 ...
                 // Same staff update
                 newStaffData[currentStaffIdx].tasks = newStaffData[currentStaffIdx].tasks.map(t => 
                     t.id === taskToMove.id ? { ...t, startDate: dateStr, rowOffset: newRowOffset } : t
                 );
             }
             setStaffData(newStaffData);
  ...
  return (
    <div 
        className="min-h-screen bg-white text-slate-800 flex flex-col select-none h-screen overflow-hidden"
    ...
    >
      <EditTaskModal 
        ...
        onSave={(taskId, updates) => updateTask(editModal.staffId, taskId, updates)}
      />

      <EditStaffModal
        isOpen={editStaffModal.isOpen}
        staff={editStaffModal.staff}
        onClose={() => setEditStaffModal({ ...editStaffModal, isOpen: false })}
        onSave={(staffId, updates) => updateStaff(staffId, updates)}
      />
      
      {/* Global Realtime Tooltip */}
      {tooltip && tooltip.visible && (
        <div 
            className="fixed z-[9999] bg-gray-900 text-white text-xs px-2 py-1.5 rounded shadow-lg pointer-events-none space-y-0.5"
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white z-50 relative shadow-sm shrink-0">
        <div>
          <h1 className="text-xl font-bold text-gray-900">人员排期与负荷</h1>
          <p className="text-xs text-gray-500 mt-1">拖动图表滑动 • 拖拽任务任意移动 • 左右边界自动滚屏 • 双击编辑 • 右键管理</p>
        </div>
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg">
             {(['month', 'quarter', 'year'] as ViewMode[]).map(mode => (
                 <button 
                   ...
                   className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${viewMode === mode ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-900'}`}
                 >
      ...
      >
        ...
        <div className="sticky top-0 z-40 flex border-b border-gray-200 bg-gray-50 shadow-sm header-row">
            <div 
              className="w-64 flex-shrink-0 p-3 border-r border-gray-200 text-xs font-semibold text-gray-500 flex items-center bg-gray-50"
            >
              人员 / 饱和度
            </div>
            <div 
            ...
            >
              {headers.map((h, i) => (
                <div 
                  key={i} 
                  className={`flex flex-col items-center justify-center py-2 border-r border-gray-200 text-xs ${
                      h.isToday ? 'bg-blue-100/50' : h.isWeekend ? 'bg-gray-200/50' : ''
                  }`}
                >
        ...
        <div className="relative">
             {staffData.map(staff => (
              <StaffRow 
                ...
                viewMode={viewMode}
                onStaffDragStart={handleStaffDragStartWithRef}
                onStaffDragEnter={handleStaffLiveSort}
            ...
            {quickJumpDir && (
                <div className="absolute top-1/2 -translate-y-1/2 z-50 pointer-events-none w-full flex justify-between px-10 pl-[280px]">
                    {quickJumpDir === 'left' && (
                        <button 
                            className="bg-indigo-600 text-white p-3 rounded-full shadow-lg hover:bg-indigo-700 pointer-events-auto animate-bounce"
                            onClick={() => jumpToData('left')}
                            title="跳转到左侧数据"
                        >
        ...
        {contextMenu && (
            <div 
            ref={contextMenuRef}
            className="absolute bg-white border border-gray-200 shadow-xl rounded-lg py-1 z-[100] w-48 text-sm"
            style={{ top: contextMenu.y, left: contextMenu.x }}
            >
            {contextMenu.type === 'general' && (
                <button 
                onClick={addStaff}
                className="w-full text-left px-4 py-2 hover:bg-indigo-50 text-gray-700 flex items-center gap-2"
                >
                <UserPlus size={14} />
                <span>新增人员</span>
                </button>
            ...
            {(contextMenu.type === 'staff' || contextMenu.type === 'row') && contextMenu.staffId && (
                <>
                <button 
                    onClick={() => contextMenu.staffId && addTask(contextMenu.staffId)}
                    className="w-full text-left px-4 py-2 hover:bg-indigo-50 text-gray-700 flex items-center gap-2"
                >
                ...
                <button 
                    onClick={() => contextMenu.staffId && deleteStaff(contextMenu.staffId)}
            ...
            {contextMenu.type === 'task' && (
                ...
                <button 
                  onClick={() => contextMenu.staffId && contextMenu.taskId && deleteTask(contextMenu.staffId, contextMenu.taskId)}
                  className="w-full text-left px-4 py-2 hover:bg-red-50 text-red-600 flex items-center gap-2"
                >
                  <Trash2 size={14} />
                  <span>删除任务</span>
