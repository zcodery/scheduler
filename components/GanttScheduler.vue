<template>
  <div
    class="min-h-screen flex flex-col select-none h-screen overflow-hidden bg-white text-slate-800 dark:bg-slate-900 dark:text-slate-100"
    role="region"
    :aria-label="title || 'Gantt Scheduler'"
  ...
  >
    ...
    <div v-if="tooltip && tooltip.visible" class="fixed z-[9999] bg-gray-900 text-white text-xs px-2 py-1.5 rounded shadow-lg pointer-events-none space-y-0.5" :style="{ left: tooltip.x + 'px', top: tooltip.y + 'px' }" aria-live="polite">
      <div>开始: {{ tooltip.startDate }}</div>
      <div>结束: {{ tooltip.endDate }}</div>
      <div>工期: {{ tooltip.duration }} 天</div>
    </div>

    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-white dark:bg-slate-900 z-50 relative shadow-sm shrink-0" role="banner">
      <div>
        <slot name="title">
          <h1 class="text-xl font-bold text-gray-900 dark:text-slate-100">{{ title || '人员排期' }}</h1>
        </slot>
        <slot name="description">
          <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">{{ description || '拖动图表滑动 • 双击编辑 • 右键管理' }}</p>
        </slot>
      </div>
      <div class="flex items-center gap-4">
        <el-button-group aria-label="视图模式">
          <el-button v-for="m in viewModes" :key="m" size="mini" :type="viewMode === m ? 'primary' : 'default'" @click="viewMode = m">{{ modeLabel(m) }}视图</el-button>
        </el-button-group>
        <div class="rs-navbox" role="group" aria-label="时间导航">
          <i class="el-icon-arrow-left text-sm cursor-pointer hover:text-orange-600" @click="nav('prev')" aria-label="上一段"></i>
          <span class="rs-navbox-label" :aria-live="'polite'">{{ currentLabel }}</span>
          <i class="el-icon-arrow-right text-sm cursor-pointer hover:text-orange-600" @click="nav('next')" aria-label="下一段"></i>
        </div>
        <el-tag v-if="readonly" type="info" effect="plain" size="mini" aria-label="只读模式">只读模式</el-tag>

        <div class="flex items-center">
          <el-button size="mini" @click="jumpToToday" aria-label="定位今天">定位今天</el-button>
          <el-button size="mini" @click="collapseAll" aria-label="折叠全部">折叠全部</el-button>
          <el-button size="mini" @click="expandAll" aria-label="展开全部">展开全部</el-button>
        </div>
      </div>
    </div>

    <div ref="containerRef" class="flex-1 overflow-y-auto overflow-x-hidden relative" @mousedown="onPanMouseDown" @contextmenu.prevent @wheel="onWheel" role="grid" :aria-rowcount="displayStaffs.length" :aria-colcount="headers.length">
      <div class="sticky top-0 z-40 flex border-b border-gray-200 bg-gray-50 dark:bg-slate-800 shadow-sm header-row" role="row">
        <div class="w-64 flex-shrink-0 p-3 border-r border-gray-200 text-xs font-semibold text-gray-500 dark:text-slate-300 flex items-center bg-gray-50 dark:bg-slate-800" @contextmenu.prevent="onHeaderSidebarContext" role="columnheader">人员 / 饱和度</div>
        <div class="flex-1 grid overflow-hidden" :style="{ gridTemplateColumns: `repeat(${headers.length}, 1fr)` }">
          <div v-for="(h, i) in headers" :key="i" :class="['flex flex-col items-center justify-center py-2 border-r border-gray-200 text-xs', h.isToday ? 'bg-blue-100/50' : h.isWeekend ? 'bg-gray-200/50' : '']" role="columnheader">
            <span :class="['font-medium whitespace-nowrap', h.isToday ? 'text-blue-600' : 'text-gray-700 dark:text-slate-200']">{{ h.label }}</span>
            <span class="text-[10px] text-gray-400 dark:text-slate-400 mt-0.5 whitespace-nowrap">{{ h.subLabel }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="relative">
        <draggable v-model="staffData" item-key="id" :disabled="readonly" handle=".rs-staff-handle" :animation="150">
          <StaffRow
            ...
            @open-edit-task="openEditTask(s)"
            @open-edit-staff="openEditStaff(s)"
            @resize-start="handleResizeStart"
            @task-mouse-down="handleTaskMouseDown"
            @update-staff="updateStaff"
            @add-task-at="addTaskAtDate"
      ...
      <div class="pointer-events-none absolute top-0 bottom-0" :style="{ left: 260 + todayLineCalc() + 'px' }" aria-hidden="true">
        <div class="w-px h-full bg-indigo-500/50"></div>
      </div>

      <div v-if="contextMenu && menuVisible" ref="contextMenuRef" class="fixed z-[1000]" :style="{ left: (contextMenu.clientX || 0) + 2 + 'px', top: (contextMenu.clientY || 0) + 2 + 'px' }" role="dialog" aria-modal="true">
        <div class="rs-menu">
...
export default {
  ...
  mounted() {
    ...
    setDark()
    if (mm && mm.addEventListener) mm.addEventListener("change", setDark)

    const container = this.$refs.containerRef as HTMLDivElement
    if (container && (window as any).ResizeObserver) {
      this.resizeObserver = new (window as any).ResizeObserver(() => {
        this.viewStartDate = this.viewStartDate + 0 // trigger recompute
      })
  ...
  methods: {
    applyTaskProp() {
      const ok = this.validateTaskData(this.task)
      this.staffData = ok ? (this.task as Staff[]) : []
    },
    validateTaskData(val: any): val is Staff[] {
      ...
      for (const s of val) {
        ...
        for (const t of s.tasks) {
          ...
          if (typeof t.startDate !== "string" || typeof t.duration !== "number") return false
        }
      }
      return true
    },
    onKeyDown(e: KeyboardEvent) {
      if (e.key === "ArrowLeft") {
        e.preventDefault()
        this.viewStartDate = this.viewStartDate - this.viewDurationMs * 0.05
