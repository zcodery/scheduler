...
export const StaffRow: React.FC<StaffRowProps> = ({ 
  ...
  return (
    <div 
        ...
        className={`flex border-b border-gray-100 bg-white transition-all duration-300 ease-in-out ${rowHeightClass}`}
        style={dynamicHeight}
    >
      {/* Left Column: Staff Info */}
      <div 
        className="w-64 flex-shrink-0 p-4 border-r border-gray-200 flex flex-col justify-center select-none bg-white relative group z-20 transition-colors hover:bg-gray-50/50"
        onContextMenu={(e) => onContextMenu(e, 'staff', staff)}
        onDragOver={handleStaffDragOver}
        onDragEnter={handleStaffDragEnter}
      ...
      >
        <div 
            ...
            draggable
            onDragStart={(e) => { e.stopPropagation(); onStaffDragStart(e, staff.id); }} // Stop prop to keep drag clean
        >
        ...
        <div className="flex items-center gap-3 pl-4">
          <button 
            onClick={(e) => { e.stopPropagation(); onToggleCollapse(staff.id); }}
          ...
          {staff.avatar ? (
             <img 
                src={staff.avatar} 
                alt={staff.name} 
                className="w-10 h-10 rounded-full object-cover border border-gray-200 flex-shrink-0 select-none"
             />
          ) : (
            <div 
          ...
          <div className="flex-1 min-w-0">
            {editingField === 'name' ? (
              <input 
                ...
                onBlur={(e) => {
                  if (e.target.value.trim()) onStaffUpdate(staff.id, { name: e.target.value });
            ...
            {editingField === 'role' ? (
              <input 
                ...
                onBlur={(e) => {
                  if (e.target.value.trim()) onStaffUpdate(staff.id, { role: e.target.value });
                  setEditingField(null);
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') e.currentTarget.blur();
      ...
      >
           ...
           {headers.map((h, i) => (
             <div 
                ...
                className={`border-r border-gray-100 h-full ${
                  h.isToday ? 'bg-blue-50/60' : h.isWeekend ? 'bg-gray-50/80' : ''
                }`}
             />
           ))}
        </div>
              ...
              return (
                ...
                >
                  <TaskCard 
                    task={task}
                    viewMode={viewMode}
                    onMouseDown={(e) => onTaskMouseDown(e, task, staff.id)}
                    onContextMenu={(e) => onContextMenu(e, 'task', staff, task)}
                    onUpdate={(newName) => onTaskUpdate(staff.id, task.id, { name: newName })}
                    onResizeStart={(e, dir, t) => onResizeStart(e, dir, t, staff.id)}
                  />
                </div>
              );
            })}
        ...
        {staff.isCollapsed && staff.tasks.length > 0 && (
           <div className="absolute inset-0 flex items-center px-2">
              <span className="ml-2 text-xs text-gray-400 whitespace-nowrap bg-white/80 px-2 rounded-full border border-gray-200">
                {staff.tasks.length} 任务
              </span>
           </div>
        )}
      </div>
